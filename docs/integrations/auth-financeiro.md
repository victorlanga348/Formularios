# Integração: Autenticação, Trava Financeira e Resiliência
...
- **`SOMENTE_BOLETO`**: O estudante possui saldo devedor > 0 no banco legado ou o sistema está em fallback de proteção financeira. O acesso a rotas de `/matricula/*` é sumariamente rejeitado (HTTP 403 Forbidden). O estudante é confinado a `/divida/liquidar`.
