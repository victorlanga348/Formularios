...
### 3.1 Regra P0: Trava Financeira & Resiliência
- **Critério:** Consulta de saldo no banco legado. Se `saldoDevedor > 0`, gera claim `SOMENTE_BOLETO`.
- **Efetivação Reativa no Login:** Ao detectar `saldoDevedor === 0`, o sistema altera o status de qualquer matrícula `PENDENTE` do estudante para `EFECTIVADA`.
- **Fail-Closed:** Em caso de indisponibilidade externa sem garantia em cache, aplica fallback restritivo (`SOMENTE_BOLETO`).

### 3.2 Regra P1: Validação de Calouro
A inscrição semestral online não é aplicável a estudantes no primeiro semestre curricular:
```typescript
...
### 3.4 Regra P3: Barreira de Ciclo (Hard Stop)
...
### 3.5 Regra P4: Próximo Passo Lógico (PPL)
...
### 3.6 Regra P5: Inscrição Híbrida e Desmarcação em Cascata
